...
function statusClass(status) {
  switch (status) {
    case "EN_ATTENTE":
    case "EN_COURS":
      return "statusChip progress";
    case "CONFIRMEE":
    case "EXPEDIEE":
      return "statusChip ok";
    case "LIVREE":
      return "statusChip done";
    case "ANNULEE":
      return "statusChip bad";
    default:
      return "statusChip";
  }
}
              ...
              {orders.map((order) => (
                <article className="orderCard" key={order.id}>
                  <div className="orderTop">
                    <div>
                      <h3>{order.referenceCommande}</h3>
                      <p>{fmtDate(order.dateCommande)}</p>
                    </div>

                    <div className={statusClass(order.statutCommande)}>
                  ...
                  <div className="orderMetaGrid">
                    ...
                    <div><span>Payment</span><strong>{order.modePaiement || "-"}</strong></div>
                    ...
                    {order.lignes?.map((line) => (
                      <div className="orderLine" key={line.id}>
                        <div>
                          ...
                          <p>
                            {[line.couleurNom, line.taillePointure].filter(Boolean).join(" • ") || "Product option"}
                          </p>
                        </div>

                        <div className="orderLineRight">
                          <span>x{line.quantite}</span>
                          <strong>{fmtPrice(line.sousTotal)}</strong>
                        </div>
                      </div>
                    ))}
                  </div>

// src/pages/admin/WorkersPage.jsx
...
const EMPTY_FORM = {
  ...
  email: "",
  password: "",
  role: "VENDEUR", // ✅ Use DB name default
  statutCompte: "ACTIVE",
};

// ✅ Use exact DB names for values
const ROLE_OPTIONS = ["Gestionnaire de catalogue", "Responsable e-commerce", "Administrateur"];
...
export default function WorkersPage() {
  ...
  async function loadList(keepSelection = true) {
    ...
    try {
      ...
      // ✅ Filter using the DB role names
            // ✅ Filter using the display role names returned by the backend
      ...
      setError(e?.response?.data?.message || "Cannot load workers");
      setRows([]);
      setSelected(null);
      setHistoryRows([]);
    } finally {
  ...
  async function deleteUser(id) {
    if (!isAdminGeneral) return;
    if (!window.confirm("Delete this worker permanently? This action cannot be undone.")) return;
  ...
  async function createUser(e) {
    ...
    try {
      ...
      setError(e?.response?.data?.message || "Create worker failed");
    } finally {
      setCreating(false);
    }
  }
  ...
  const header = (
    <div className="flex flex-wrap align-items-center justify-content-between gap-2 p-3">
      <div>
        <span className="text-xl text-900 font-bold">Workers</span>
        <div className="text-sm text-500 mt-1">{rows.length} workers found</div>
      </div>
    ...
    <>
      ...
      <dialog ref={profileDialogRef} className="admDialog admDialogWide" onClick={onBackdropClose}>
        <div className="admDialogHead">
          <div className="admDialogTitle">Worker profile</div>
          ...
          <div className="admDialogBody">
            ...
            <div className="admDialogSection">
              ...
              <div className="admActionGrid">
                <button className="admBtn mini" disabled={busyId === selected.id || selected.statutCompte === "ACTIVE"} onClick={() => setStatus(selected.id, "ACTIVE")}>Activate</button>
                <button className="admBtn mini" disabled={busyId === selected.id || selected.statutCompte === "BLOCKED"} onClick={() => setStatus(selected.id, "BLOCKED")}>Block</button>
                <button className="admBtn mini" disabled={busyId === selected.id || selected.statutCompte === "DISABLED"} onClick={() => setStatus(selected.id, "DISABLED")}>Disable</button>
              </div>
            </div>
      ...
      <dialog ref={createDialogRef} className="admDialog" onClick={onBackdropClose}>
        <form className="admDialogBody admForm" onSubmit={createUser}>
          <div className="admDialogHead">
            <div className="admDialogTitle">Create worker</div>
          ...
          <div className="admFormActions">
            ...
            <button className="admBtn primary" type="submit" disabled={creating}>{creating ? "Creating..." : "Create worker"}</button>
          </div>
        </form>
      </dialog>
    </>

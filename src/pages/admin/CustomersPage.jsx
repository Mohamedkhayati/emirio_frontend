...
const ROLE_OPTIONS = ["USER", "VENDEUR", "ADMIN_GENERAL", "CONTROLEUR"];
...
export default function CustomersPage() {
  ...
  async function createUser(e) {
    ...
    try {
      ...
      setError(e?.response?.data?.message || "Create account failed");
    } finally {
      setCreating(false);
    }
  }
  ...
  const header = (
    <div className="flex flex-wrap align-items-center justify-content-between gap-2 p-3">
      <div>
        <span className="text-xl text-900 font-bold">Customer List</span>
        <div className="text-sm text-500 mt-1">{rows.length} customers found</div>
      </div>
  ...
  return (
    <>
      ...
      <dialog
        ...
        className="admDialog admDialogWide"
      ...
      >
        <div className="admDialogHead">
          ...
          <button
            ...
            type="button"
            onClick={() => closeDialog(profileDialogRef)}
          >
            Close
          </button>
        </div>
